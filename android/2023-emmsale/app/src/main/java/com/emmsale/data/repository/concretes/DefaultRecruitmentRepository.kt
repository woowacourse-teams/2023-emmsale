...
class DefaultRecruitmentRepository(
    ...
    override suspend fun postRecruitment(
        ...
        return when (val response = recruitmentService.postRecruitment(eventId, requestBody)) {
            is Success -> Success(getRecruitmentIdFromHeaderLocation(response))
    ...
    private fun getRecruitmentIdFromHeaderLocation(result: Success<Unit>): Long {
    ...
    override suspend fun checkIsAlreadyPostRecruitment(
        eventId: Long,
    ): ApiResponse<Boolean> = recruitmentService.checkIsAlreadyPostRecruitment(eventId, myUid)

    override suspend fun reportRecruitment(
        recruitmentId: Long,
        authorId: Long,
        reporterId: Long,
    ): ApiResponse<Unit> = recruitmentService.reportRecruitment(
    ...
    companion object {
        ...
        private const val NOT_LOGIN_ERROR_MESSAGE = "[ERROR] 로그인 되지 않아서 같이가요 정보를 가져올 수 없어요"
        private const val LOCATION_HEADER_NOT_VALID = "[ERROR]  헤더 값이 제대로 오지 않았어요. 서버와 명세를 맞춰보세요."
    }
}
