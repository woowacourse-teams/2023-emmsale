...
public class GlobalControllerAdvice {
  ...
  private final Logger warnLogger = LoggerFactory.getLogger("globalWarnLogger");
  private final Logger errorLogger = LoggerFactory.getLogger("globalErrorLogger");
  ...
  public ResponseEntity<ExceptionResponse> handleException(final BaseException e) {
    ...
    if (type.httpStatus().is5xxServerError()) {
      errorLogger.error("[ERROR] MESSAGE : {}, 로그 캡처와 함께 서버 개발자에게 연락주세요 : ", type.errorMessage(),
          e);
      return new ResponseEntity<>(ExceptionResponse.from(e), HttpStatus.INTERNAL_SERVER_ERROR);
    }

    warnLogger.warn("[WARN] MESSAGE: {}", type.errorMessage());
  ...
  public ResponseEntity<ExceptionResponse> handleValidationException(
    ...
    warnLogger.warn("[WARN] MESSAGE: {}", message);
  ...
  public ResponseEntity<ExceptionResponse> handleException(final Exception exception) {
    errorLogger.error("[ERROR] 로그 캡처와 함께 서버 개발자에게 연락주세요 : ", exception);
