...
class CommentTest {
  ...
  static Stream<Arguments> commentCandidate() {
    ...
    final Feed feed = new Feed(event, writer, "피드 제목", "피드 내용");

    final Comment comment1 = Comment.createRoot(feed, writer, "댓글 내용");
    final Comment comment2 = Comment.createRoot(feed, writer, "댓글");
    comment2.delete();
    final Comment comment3 = Comment.createRoot(feed, writer, "삭제된 댓글입니다.");
  ...
  void init() {
    final Event event = EventFixture.인프콘_2023();
    feed = new Feed(event, writer, "피드 제목", "피드 내용");
  ...
  void getContentAndHideIfBlockedMemberWithEmptyBlockedMemberList() {
    ...
    final Comment comment = Comment.createRoot(feed, blockedMember, expectContent);
  ...
  void test_createChild_not_create_child_child_comment() throws Exception {
    ...
    final Comment root = Comment.createRoot(feed, member, "부모내용");
    final Comment child1 = Comment.createChild(feed, root, member, "자식내용1");
    final Comment child2 = Comment.createChild(feed, root, member, "자식내용2");

    //when & then
    assertThatThrownBy(() -> Comment.createChild(feed, child1, member, "자자식댓글1"))
